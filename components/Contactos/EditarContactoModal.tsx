...
export const EditarContactoModal = ({ openvalueedit, getContactos, resetFiltros, setOpenValueEdit, usuarioactual }: any) => {

    const { isOpen, onOpen, onOpenChange } = useDisclosure();
    const [errordata, setErrorData] = useState(false);
    const [errormsg, setErrorMsg] = useState("");

    const [nombreedit, setNombreEdit] = useState(usuarioactual.nombre);
    const [rfcedit, setRFCEdit] = useState(usuarioactual.rfc);

    const [tipoCuenta, setTipoCuenta] = useState(usuarioactual.tipoCuenta);
    const [nombreTipoCuenta, setNombreTipoCuenta] = useState(usuarioactual.nombreTipoCuenta);
    const [cuenta, setCuenta] = useState(usuarioactual.cuenta);

    const [loading, setLoading] = useState(false);

    const [openvaluemodal, setOpenValueModal] = useState(openvalueedit);

    const [bancos, setBancos] = useState<Elemento[]>([]);
    const [tiposdecuentas, setTiposDeCuentas] = useState<Elemento[]>([]);
    const [idBanco, setIdBanco] = useState(usuarioactual.idBanco);
    const [nombreBanco, setNombreBanco] = useState(usuarioactual.nombreBanco);

    const router = useRouter();

    useEffect(() => {

        if( bancos.length <= 0 ) getBancos();
        if( tiposdecuentas.length <= 0 ) getTiposDeCuentas();
    ...
    const editarContacto = async () => {
        ...
        const datasend = JSON.stringify({
            ...
            fechaRegistroK: usuarioactual.fechaRegistroK,
            estatusK: usuarioactual.estatusK
        });

        const token = localStorage.getItem("token_axnweb");
        const baseURL = config.baseUrlAPI + "contactos/edit";
        ...
        <>
            ...
            >
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1 font-montserrat_bold">Editar Contacto</ModalHeader>
                            <ModalBody>
                            ...
                            <ModalFooter>
                                {!loading ?
                                    <div>
                                        <Button color="danger" variant="flat" onPress={() => { setOpenValueEdit(false), setErrorData(false) }}>
                                            <p className='font-montserrat'>Cancelar</p>
                                        </Button>
                                        <span> </span>
                                        <Button color="primary" onPress={() => { onClose(); editarContacto(); }}>
                                            <p className='text-white font-montserrat'>Guardar</p>
                                        </Button>
